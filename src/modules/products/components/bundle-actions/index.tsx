...
export default function BundleActions({ bundle }: BundleActionsProps) {
  ...
  const allVariantsSelected = useMemo(() => {
    return bundle.items.every((item, index) => {
      ...
      if (item.optional) return true // optional item can be skipped
      return selected !== undefined // required item must be selected
    })
    ...
    await addBundleToCart({
      ...
      items: bundle.items
        .map((item, index) => {
          ...
          // Skip optional unselected
          if (!variant && item.optional) {
            return null
          }

          // Error on missing required
          if (!variant) {
            throw new Error(`Missing variant for required item: ${item.id}`)
          }
        ...
        .filter(
          (item): item is { item_id: string; variant_id: string } =>
            item !== null
        ),
    })
    setIsAdding(false)
